...
def get_env_variable(key: str, default: str = None, required: bool = False) -> str:
    """
    安全读取环境变量的通用函
    :param default: 如果找不到变量，返回的默认值
    :param required: 为 True 且找不到变量，程序会直接报错中断
    :return: 环境变量的值
    """
    load_dotenv()
    value = os.getenv(key)
    if value is None:
        if required:
            raise KeyError(f"Critical configuration missing: The necessary environment variable '{key}' could not be found. Please check your .env file!")
        return default
    return value