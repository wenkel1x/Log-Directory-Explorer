...
def _configure_common(app):
    raw_password = get_env_variable("DB_PASSWORD", required=True)
    db_port = get_env_variable("DB_PORT", required=True)
    safe_password = urllib.parse.quote_plus(raw_password)

    app.config['SQLALCHEMY_DATABASE_URI'] = f'mysql+pymysql://root:{safe_password}@127.0.0.1:{db_port}/log_system?charset=utf8mb4'
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

    app.config['SQLALCHEMY_BINDS'] = {
        'log_system': f'mysql+pymysql://root:{safe_password}@127.0.0.1:{db_port}/log_system?charset=utf8mb4',
        'ict_log_system': f'mysql+pymysql://root:{safe_password}@127.0.0.1:{db_port}/ict_log_system?charset=utf8mb4'
    }
