...
class IctLogAgent:
    def __init__(self, args):
        ...
        # ========================================================
        # 1. Fixture No: Starts with AA or AB, followed by alphanumeric
        self.re_fixture = re.compile(r'^(AA|AB)[A-Z0-9]+$', re.IGNORECASE)
        # 2. 11-digit Part Number (PN): Starts with 1, 2, or 3, 11 digits total
        self.re_pn_standard = re.compile(r'^[123][A-Z0-9]{10}$')
        # 3. Standard test_history filename: SN_STATUS_STAGE_TIMESTAMP
        self.re_history_parse = re.compile(r'^([A-Z0-9\-]+)_(PASS|FAIL)_(ICT|OST)_(\d{8,14})(?:\.[a-z0-9]+)?$', re.IGNORECASE)
        # 4. For files directly under test_history without PN: SN-time-STATUS.dcl
        self.re_history_dcl_parse = re.compile(r'^([A-Z0-9\-]+)-(\d{8,14})-(PASS|FAIL)(?:\.[a-z0-9]+)?$', re.IGNORECASE)
        # 5. Standard testresult filename: SN_TIMESTAMP
